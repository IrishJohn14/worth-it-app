...
function formatHours(hours) {
  if (hours < 1 / 60) return `${Math.round(hours * 3600)}s of work`;
  if (hours < 1) {
    const mins = Math.round(hours * 60);
    return `${mins} min${mins !== 1 ? "s" : ""} of work`;
  }
  const h = Math.floor(hours);
  const m = Math.round((hours - h) * 60);
  if (m === 0) return `${h} hr${h !== 1 ? "s" : ""} of work`;
  return `${h}h ${m}m of work`;
}
...
export default function App() {
  ...
  const hoursColor = (hours) => {
    if (hours < 1) return "#4ade80";
    if (hours < 4) return "#facc15";
    if (hours < 8) return "#fb923c";
    return "#f87171";
  };
 
  const ring = result ? hoursColor(result.hours) : "#6366f1";
    ...
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        @keyframes pulse { 0%,100%{transform:scale(1);opacity:.15} 50%{transform:scale(1.4);opacity:.3} }
        ...
        .tab-btn { background: none; border: none; cursor: pointer; padding: 10px 18px; font-family: 'DM Mono', monospace; font-size: 13px; letter-spacing: .05em; transition: all .2s; border-bottom: 2px solid transparent; }
        .tab-btn.active { border-bottom-color: #6366f1; color: #a5b4fc; }
        .tab-btn:not(.active) { color: #555; }
        .tab-btn:hover:not(.active) { color: #888; }
        .primary-btn { background: #6366f1; border: none; color: white; padding: 14px 32px; border-radius: 12px; font-family: 'DM Mono', monospace; font-size: 14px; cursor: pointer; letter-spacing: .08em; transition: all .2s; font-weight: 500; }
        .primary-btn:hover { background: #7c7ffa; }
        .ghost-btn { background: transparent; border: 1px solid #333; color: #888; padding: 12px 24px; border-radius: 10px; font-family: 'DM Mono', monospace; font-size: 13px; cursor: pointer; transition: all .2s; }
        .ghost-btn:hover { border-color: #555; color: #ccc; }
        .field { display: flex; flex-direction: column; gap: 8px; }
        .label { font-size: 11px; letter-spacing: .12em; color: #555; text-transform: uppercase; }
        .input-base { background: #13131a; border: 1px solid #222; border-radius: 10px; color: #f0f0f5; font-family: 'DM Mono', monospace; font-size: 15px; padding: 13px 16px; transition: border .2s; width: 100%; }
        ...
        .decision-btn { flex: 1; padding: 16px; border-radius: 12px; font-family: 'DM Mono', monospace; font-size: 14px; cursor: pointer; border: none; font-weight: 500; letter-spacing: .05em; transition: all .2s; }
      `}</style>
